...
# census.csv = read_csv("acs.csv")
# census.csv <- na.omit(census)
# str(census)
# str(census.csv)
...
df <- left_join(df, pop_density, by = c("county","state"))
skim(df)


df <- df %>%  mutate(percent_child = 100*(child_crime /total_pop)) %>% 
              mutate(percent_partner = 100*(partner_crime /total_pop)) %>%
              mutate(percent_stranger = 100*(stranger_crime /total_pop)) %>%
              mutate(percent_burglarly = 100*(burglary_home_stranger /total_pop)) %>% 
              mutate(percent_closed_25 = share_all_closed_25 * 100) %>% 
              mutate(tfHH_18_percent = (tfHH_18 / tHH)*100) %>% 
              mutate(tfHH_65_percent = (tfHH_65 / tHH)*100) %>% 
              mutate(clf_percent = (clf/total_pop)*100) %>% 
              mutate(clf_ue_percent = (clf_ue/total_pop)*100)
